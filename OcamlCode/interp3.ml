...
This code is based on the provided solution of interpter part 2
...
type command =
  ...
  | Call
  | Throw
  | TryC of (command list * command list)
...
and deffun () = 
  ((keyword "DefFun") >> (name << ws)) +++ (name << ws) +++ ( many' command )<< (keyword "End") >|=
  (fun (( n1,  n2), cmds6) -> DefFun (N n1,N n2,cmds6))

and tryc () = 
  ((keyword "Try") >> many' command) +++
  ((keyword "Catch") >> many' command) << (keyword "End") >|=
  (fun (cmds4, cmds5) -> TryC (cmds4, cmds5))
...
type value =
  ...
  | N_val of string
  | U_val
  | F_val of (env * value * value * command list)
...
let rec run (p : prog) (st : stack) (e : env) (log : string list) :
  ...
  | TryC (cmds1, cmds2) :: rest ->
    (match run cmds1 [] e log with
     | (log1, Ok st1) -> run rest (st1 @ st) e (log @ log1)
     | (log1, r) -> run (cmds2 @ rest) ( I_val (to_int_result r ) :: st) e (log @ log1) )

  | DefFun (str1, str2, cmds) :: rest ->
    (match (str1, str2, cmds) with
     | (N str1, N str2, cmds) -> run rest st (put e str1 (F_val (e, N_val str1, N_val str2, cmds))) log
     | _ -> log, TypeError
    )
  | Call :: rest ->
    (match st with 
     | [] -> log, StackError
     | _ :: [] -> log, StackError
     | x :: F_val (env, N_val name, N_val para, c) ::t -> 
       ( match run c [] (put (put env name (F_val (env, N_val name, N_val para,c))) para x) [] with
         | (log1, Ok (s1 :: _)) -> run rest (s1 :: t) e (log1 @ log)
         | (log1, Ok []) -> (log @ log1), TypeError
         | (log1, r) -> (log1 @ log), r
       )
