...
//static int Index = 0;  // current thread

static struct thread_control_block *current;
static struct thread_control_block *header; 
...
struct thread_control_block {
	...
	void  *stack;
	jmp_buf buf;
	struct thread_control_block *prev, *next;
...
static void schedule(int signal)
{
	 ...
	 */
		...
		if (!setjmp (current->buf)){
			...
			while ((current->next != NULL) && (current->next->state != TS_READY)   ){
			...
			// while ( (current->next != NULL) && (current->state != TS_READY) ){
			// 	current = current->next;
			// }
...
static void scheduler_init()
{
	 ...
	 */
    header = (struct thread_control_block*) malloc (sizeof(struct thread_control_block));
	header->id = 0;
    ...
    // get from https://www.informit.com/articles/article.aspx?p=23618&seqNum=14
...
int pthread_create( pthread_t *thread, const pthread_attr_t *attr, void *(*start_routine) (void *), void *arg) {
	...
	new->id = ID; *thread = ID++;
	new->stack = (void*) malloc (THREAD_STACK_SIZE);
	long unsigned int *p;
	p = new->stack + THREAD_STACK_SIZE;
	p[-1] = (long unsigned int) &pthread_exit;
    p--;
	//setjmp(tcb[number]->buf);

	new->buf[0].__jmpbuf[JB_RSP] = ptr_mangle ((long unsigned int) p);
    new->buf[0].__jmpbuf[JB_PC] = ptr_mangle ((long unsigned int) start_thunk);
	new->buf[0].__jmpbuf[JB_R12] = (long unsigned int) start_routine;
	new->buf[0].__jmpbuf[JB_R13] = (long unsigned int) arg;

	new->state = TS_READY;
	
    struct thread_control_block *trav = header;
